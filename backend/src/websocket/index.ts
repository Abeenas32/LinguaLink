            ...
            try {
              ...
              const translationPromises = receivers.map(async (receiver) => {
                try {
                  ...
                  // Store i.e. update translated message in DB PARALLELY (with translatedText field)
                  const translatedMessage = await prisma.message.update({
                    where: { id: savedMessage.id }, 
                    data: {
                      translatedText: translation.success
